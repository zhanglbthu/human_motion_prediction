...
def test(model, epoch):
    ...
    for i, (data, _) in tqdm(enumerate(data_gen)):
        ...
        if args.mode == 'train' and (i >= 500 and (epoch + 1) % 50 != 0 and (epoch + 1) < cfg.num_epoch - 100):
            break
        num_samples += 1
        gt = data[..., 1:, :].reshape(data.shape[0], data.shape[1], -1)[:, t_his:, :] # [1, 100, 48]
        gt_multi = traj_gt_arr[i] # [125, 100, 48]
        if gt_multi.shape[0] == 1:
            continue
    
        pred = get_prediction(data, model, sample_num=1, num_seeds=num_seeds, concat_hist=False)
        pred = pred[:,:,t_his:,:] # [1, 50, 100, 48]
...
if __name__ == '__main__':
    ...
    if args.iter > 0:
        ...
        cp_path = (cfg.model_path % args.iter).replace('.p', '_cpu.p')
        print('loading model from checkpoint: %s' % cp_path)
        model_cp = pickle.load(open(cp_path, "rb"))
        model_cp = recursive_to_cuda(model_cp, device=device)
